...
with code_col:
    ...
    st.info(
        ...
        "한국은행 연평균 기준금리가 **1.56%(2019년)에서 0.64%(2021년)로 급락**하는 초저금리 국면에서 청년층의 평균 대출 잔액이 **557.3만 원에서 696.5만 원으로 가파르게 급증**한 흐름이 실증 데이터로 입증됩니다.\n\n"
...
with code_col2:
    ...
    st.info(
        ...
        "**2. 언론 프레임의 왜곡 검증**:\n"
        "실제 2021년 청년 가구 표본 데이터의 사유별 금융부채 총량을 상호 비교해 본 결과, 주거 안정을 위한 **'거주주택 구입(영끌)' 목적의 부채 총량(약 80억 7천만 원)**은 대중 언론이 지적하던 '주식/자산투자' 목적의 부채 총량(약 41억 6천만 원)보다 정확히 2배 더 거대했습니다. > 즉, 청년 부채 폭발의 실체는 무모한 투기 성향 때문이 아니라, 당시 자산 가격 폭등 속에서 생존을 위해 선택해야 했던 '폭등하는 주거 비용에 대한 생존형 대응'이 본질이었음이 데이터 과학적으로 엄밀하게 증명됩니다."
    )
    st.markdown("**사용한 백엔드 SQL 쿼리:**")
